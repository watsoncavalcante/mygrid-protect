...
def calc_power_flow(dist_grid, fault):
    # -------------------------
    ...
    # ------------------------------------- 
    for node in dist_grid.load_nodes.values():
        node.If = np.zeros((3, 1), dtype=complex)
    
    node_to_root = list(dist_grid.load_nodes_tree.node_to_root_path(fault.columns[0])[1,:])

    for i in fault.columns:
        if i in dist_grid.load_nodes.keys():
            if i not in node_to_root: 
                dist_grid.load_nodes[i].If = np.array(fault[i])
                dist_grid.load_nodes[i].If.shape=(3,1)
            else:
                dist_grid.load_nodes[i].If = np.array(fault[i])
                dist_grid.load_nodes[i].If.shape=(3,1)
...
def Forward_Sweep(max_depth, nodes_depth_dict, dist_grid):
    ...
    while depth <= max_depth:
        ...
        for node in nodes:
            ...
            node.vp,vc = calc_vp(section.A, section.B, upstream_node.vp, node.If, node.vp)
            # ------------------------------------
