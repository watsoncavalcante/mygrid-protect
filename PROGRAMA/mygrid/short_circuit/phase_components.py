			...
			if len(isl) !=0:
				...
				for y in isl.keys():
					...
					if type(isl[y][1]) != type(None):
						if isinstance(isl[y][1].transformer,TransformerModel):
							if y == isl[y][1].n1.name:
								isl[y][1].If = izz
								isl[y][1].transformer.If2 = izz
								izz=isl[y][1].d.dot(izz)
								isl[y][1].transformer.If1 = izz


							elif  y == isl[y][1].n2.name:
								isl[y][1].If = izz
								isl[y][1].transformer.If1 = izz
								izz=isl[y][1].D.dot(izz)
								isl[y][1].transformer.If2 = izz
...
def new_phase_erase(tf_type,old_phase_erase):
	...
	pr=[0,1,2]
	if tf_type == "Dyn" and old_phase_erase != None:
